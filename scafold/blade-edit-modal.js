...
const BladeEditModal = function({path,tableName,title,id,action,actionCommit,routeAction}){
   
    let query = TableFields(tableName);

    let code = CodeGen("resources/views/" + path,"templates/blade-edit-modal.stub");
    ...
    function _renderFieldSet(label,field){
        ...
        <input class="form-control" name="${field}" placeholder="${label}" value="{{ $item->${field} }}" />
       </div>${"\n"}`;
    }
